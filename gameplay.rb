...
game = Game.new
game.secret_word = game.choose_secret_word
game.guess_string = game.make_empty_guess_string

loop do
  puts 'Would you like to load your previous game? (Y/N)'
  answer = gets.chomp.upcase
  next unless game.answer_valid?(answer)
  
  if answer == 'Y'
    game = game.load_game(answer)
    game.display_information
  elsif answer == 'N'
    puts "You've got 7 guesses"
  end
  break
end
...
loop do 
  ...
  if game.guesses_remaining == 0
    puts "You have no guesses remaining. You've lost!"
    break
  elsif game.has_won?
    puts "You guessed the word. Congradulations you've won!"
  elsif
    loop do
      next unless game.save_game(game)
